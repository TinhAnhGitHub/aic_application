...
class TagService:
    ...
    def _bonus_score_for_tags(self,user_tags: list[str], kf_tags: list[str]):
        u_norm = [strip_accents(t.strip().lower()) for t in user_tags if t and t.strip()]
        k_norm = [strip_accents(t.strip().lower()) for t in kf_tags if t and t.strip()]
    ...
    def rerank_keyframe_search_with_tags(
        ...
        if not results_search or not user_tags:
            return results_search

        out = []
        for kf in results_search:
            base = float(kf.score)
            kf_tags = self.id_to_tags.get(kf.identification, [])  
            bonus_raw = self._bonus_score_for_tags(user_tags, kf_tags) 
            bonus = bonus_raw ** gamma
            final = (1.0 - alpha) * base + alpha * bonus
            out.append(kf.__class__(**{**kf.model_dump(), "score": float(final)}))
